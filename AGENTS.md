...
このファイルは、コードを書く前に必ず読むこと。ここに書かれていないやり方は勝手に採用しない。

> ⚠⚠ **2026-09-06：実装役（Ziva / MiniMax）は使わない。** 人間の決定「minimaxはもう使わない」。
> **設計役（Claude Code）が設計から実装・検証まで通しでやる。** 以下に残る「Ziva」の記述は当時の体制のもの。
...
- エンジン: Godot 4.x
- ~~Ziva（Godotエディタに組み込むAIコーディングエージェント）~~ **使わない**（2026-09-06・人間の決定）。**設計役が直接書く**
...
│   ├── adventure/        # 冒険選択・戦闘
│   ├── inventory/        # インベントリの窓（2026-09-15・人間の決定「インベントリ専用のものをつくる」）
│   └── pomodoro/
├── scripts/
...
| `PENDING_CHESTS` | `[{instance_id, chest_id, source, obtained_at, opened, rewards}]`。`instance_id`は**その宝箱1個の一意ID**、`chest_id`は**種類**（`chests.json`のエントリ）。**`chest_type`というキーは状態に無い**（あれは`ChestScheduleEntry`の`@export`名で、中身は`chest_id`を指す）。⚠⚠ **`rewards`は「開けたあとの中身」**（2026-09-18・人間の決定「開けるときに抽選」）。**積んだ時点では空**で、`open_chest()`が振って書き込む。積む口は`grant_chest()`の1本 |
| `UNLOCKED_SCREENS` | `{screen_id: bool}` |
| `STORY` | `{current_chapter: int, stages: {stage_id: {cleared, stars}}}` |
| `FLOOR_RUN` | `{floor_id, nodes: {node_id: {layer, kind, next, cleared}}, position, visited: {node_id: bool}, torch_grade, relics: [{relic_id, character_id}], hp_carry: {character_id: int}, chest_count, bag: {item_id: int}, bag_slots, pending_loot: {item_id: int}}`。**進行中のフロア探索1本ぶん**（段階14・`PLAN_SCENARIO_MAP.md`）。`floor_id`が`""`なら**フロアに入っていない**。**フロアを降りると丸ごと捨てる**（`abandon_floor()`）ので、たいまつ・レリック・持ち越しHP・**鞄**はここで一緒に消える。⚠⚠ **`bag` / `bag_slots` / `pending_loot` は難ダンジョンの鞄と同じ形**（2026-09-18・人間の決定「難ダンジョンのインベントリをシナリオでも適用」）。読み書きは`GameManager`の`*_run_bag*` / `*_run_pending_loot*`（`RUN_KIND_FLOOR` / `RUN_KIND_DUNGEON`）の口だけ。鞄は**宝箱を`chest_id`のまま**1個1枠で持ち（見分けは`is_chest_item()`）、**ボスを倒したとき`deliver_floor_bag()`だけ**が拠点へ渡す。`kind`は`FLOOR_NODE_KIND_*`の5種（`battle` / `shop` / `relic` / `rest` / `boss`）。`relics`の`character_id`が`""`なら編成3人全員に効く。**ノードを作る口は`_build_floor_map()`の1本だけ**、**進めるかの判定は`get_available_moves()`の1本だけ** |
...
- CSVを編集したら再インポートが要る。これをしないと反映されない。
  ⚠⚠ **設計役はヘッドレスの `--import` で済ませられる**（⚠ 2026-09-22 に実測。⚠ **人間を待たない**）：
  `godot --headless --path d:\pomodoro-heroes --import`。⚠ 人間がエディタで右クリック → 再インポートしてもよい
...
- エラーが出たらまず Godot の出力を確認すること（`user://logs/godot.log`。ヘッドレスの取り方は下記「誰が取るか」）。
...
### ⚠⚠ 台帳は3つ。役割が違う（2026-09-21・人間の決定）

| ファイル | 役割 | 人間の言葉 |
|---|---|---|
| `docs/01_plan/PLAN_◯◯.md` | **とっかかり**（作る前の設計・なぜそうしたかの経緯） | ⚠ 「plan はとっかかりだと思う」 |
| `docs/NEXT_STEPS.md` | **いまの議題**（⚠ 一番上が**これからの回（仮）**・⚠ その下が終わった回 3節） | ⚠ 「nextsteps はいまの議題だと思う」 |
| ⚠⚠ **`docs/DECISIONS.md`** | ⚠⚠ **いま効いている値**（1決定＝1行） | ⚠ 「決定表を作るのはいいと思う」 |
| ⚠⚠ **`docs/HUMAN_CHECK.md`** | ⚠⚠ **人間が実機で見るもの**（⚠ 溜まった数が一目で分かる） | ⚠ 「人間が確認するものは md を作るべき」 |

- ⚠⚠ **値を書くのは `DECISIONS.md` だけ。** ⚠ `PLAN` と `NEXT_STEPS` の本文は**番号を参照する**（⚠ 値を書き写さない）
- ⚠ 理由は「状態にマスターデータを複製しない」（`CLAUDE.md` 4番）と同じ。⚠ **2か所に書くと必ず食い違う**
- ⚠ **仕様を決めるときは `DECISIONS.md` を先に見る。** ⚠ `PLAN` は経緯を読むとき
- ⚠ **覆したら行を上書きし、旧文は「覆した相手」の欄に取り消し線で1行だけ残す**（⚠ 行を増やさない）

### ⚠⚠ 進め方（2026-09-21 に実体へ合わせた。⚠ 人間「もう ziva を使わないから、あってる方式にしたい」）

**実装役（Ziva / MiniMax）はもう居ない。設計役が設計から実装・検証まで通しでやる。**
以前の `PRE_PLAN` → `EXEC` → `IMPL_LOG` の三点セットは、**渡す相手が居なくなって止まっている**
（⚠ `EXEC` は 2026-08-28 で、⚠ `IMPL_LOG` は 2026-08-17 で最後）。**型を実体に合わせた。**

```
【A】人間が依頼書・モックを渡す（PRE_PLAN_◯◯.md。⚠ 無い回もある）
【B】設計役が方針を出し、人間が裁く（⚠ 大きい変更は合意を取ってから書く）
【C】設計役が実装する。⚠ 1つ入れるごとにコミット
【D】設計役がヘッドレスで事前チェック → NEXT_STEPS に節を1つ書く
     （⚠ 決定・入ったもの・実測・報告・残り）
【E】決まった値は DECISIONS.md の行に書く（⚠ NEXT_STEPS に値を書き写さない）
```

- ⚠ **`PRE_PLAN_◯◯.md` は生きている。** ⚠ ただし中身は「人間が渡す依頼書・モック」に変わった
- ⚠⚠ **`IMPL_LOG_◯◯.md` はもう書かない。** ⚠ 「セッションをまたいで記憶しない実装役のための記録」だったが、
  ⚠ その相手が居ない。⚠ 記録の役割は **`NEXT_STEPS` の節**が果たしている。
  ⚠ **2か所に書くと食い違う**（⚠ 実際に台帳で起きた）。⚠ 過去の16本は**当時の記録として残す**
- ⚠⚠ **`EXEC_◯◯.md` は「重いものだけ」書く。** ⚠ 判定は `PLAN_HARD_DUNGEON.md` §6 と同じ言葉：

> ⚠ **その変更で、器（関数・状態のキー・画面）が作り直しになるか。**

  ⚠ なるものだけ書く（⚠ 例：鞄の数え方・層ごとの個別調整・台帳の移し替え）。
  ⚠ つまみ1つ・見た目を消すだけのものは書かない。⚠ **渡す先は人間ではなく自分**（⚠ 段取りを落とさないため）
...
## 更新履歴
- **2026-09-21（人間の決定・進め方の相談の回）**：
  - ⚠⚠ **台帳を3つに分けた**（⚠ `PLAN`＝とっかかり ／ `NEXT_STEPS`＝いまの議題 ／ **`DECISIONS.md`＝いま効いている値**）。
    ⚠ 人間の言葉「⚠ plan はとっかかりだと思う」「⚠ nextsteps はいまの議題だと思う」「⚠ 決定表を作るのはいいと思う」
  - ⚠⚠ **`IMPL_LOG` を廃止し、`EXEC` は「重いものだけ」にした**（⚠ 人間「⚠ もう ziva を使わないから、あってる方式にしたい」）。
    ⚠ 「二段構え」と「実装ログ」の2節を実体に書き直した
  - ⚠ **「着手の前に変わるはずの数字を1行書く」を足した**（⚠ 09-20 の「3区画が同じ形」を検査が通した件）
  - ⚠⚠ **「画面は撮れる」を足した**（⚠ `--headless` を外して `scenario=shot`）
  - ⚠⚠ **「実機確認は見る回にまとめる」を足した**（⚠ 人間「⚠ 頻度を減らして一気にチェックしたい」）
- **2026-09-19（人間の許可「agentsmdはいじっていいよ」）**：状態構造の表を実コードに合わせた
  - `PENDING_CHESTS` の `rewards` は**開けたあとの中身**（⚠ 積んだ時点は空・`open_chest()` が振る）
  - `FLOOR_RUN` に **鞄（`bag` / `bag_slots` / `pending_loot`）** を足した。⚠ 難ダンジョンの鞄と同じ形で、⚠ 読み書きは `GameManager` の `*_run_bag*` の口だけ
  - ⚠ 決定の経緯は `NEXT_STEPS.md` §0-UI-K・§0-UI-L
- **2026-09-15（人間の指示「５は直して」）**：フォルダ構造に `scenes/inventory/`（インベントリの窓）を足した。⚠ フォルダはこの回の人間の決定で作ったもの（`NEXT_STEPS.md` §0-UI-D-3）
- **2026-09-06（人間の決定「minimaxはもう使わない」）**：
  - **実装役（Ziva / MiniMax）を使わない体制に更新した。** 冒頭・使用技術・開発ルールの3箇所。**設計役が設計から実装・検証まで通しでやる**
  - 「エラーが出たらまずZivaのログ」を **Godot の出力（`user://logs/godot.log`）** に直した
  - **空の `plan/` フォルダを消した**（`.gitkeep` だけ・フォルダ構成に無かった。人間の決定「planふぉるだはけしていい」）
  - **残っている「Ziva」の記述**（`addons/` の説明・IMPL_LOG の理由・更新履歴）は**当時の体制の記録として残す**
...
### ⚠⚠ 着手の前に「変わるはずの数字」を1行書く（2026-09-21・人間の決定）

**検査は「壊れていないこと」しか見ていない。** 「意図した形になっているか」は別の量で、
**前者をいくら足しても後者は出ない。**

実例（2026-09-20）：分岐を区画ごとに振る変更で、検査は**赤0・通れないノード0・行き止まり0**で通した。
**人間が実機の1枚で「3つの区画が全部同じ形」を見つけた。** 検査は「割り切れるか」を見ていなかった。
...
| 変更 | 先に書く1行 |
|---|---|
| 分岐を区画ごとに振る | **区画の数で割り切れない層が 0 でなくなるはず** |
| 素材を1枠に10個 | 同じ素材15個が、鞄の **15枠 から 2枠** になるはず |
| ボスの後にショップ | 降りた直後の遷移先が `dungeon_map` から `dungeon_shop` に変わるはず |

- ⚠⚠ **書けなかったときが本番。** 「数字にできない」＝**絵か人間でしか見られない変更**だと**着手前に分かる**。
  そのまま「絵を撮る」か「見る回のリスト」へ送る（下記）
- ⚠ **全タスクに義務づけない。形骸化する。** 書くのは**生成・分布・レイアウト・数え方が変わるもの**だけ。
  見た目を消す・文言を変えるだけのものは要らない

### ⚠⚠ 画面は撮れる（2026-09-21）

**`--headless` を外して `scenario=shot` を回すと、画面が PNG で撮れる。設計役はそれを読める。**
...
- ⚠ **窓が人間の画面に出る。** ⚠ 覆う・最小化すると**描画が止まり1枚も撮れない**
- ⚠⚠ **撮る画面は必ず `SceneManager` 経由で開く。** ⚠ `add_child()` で直に足すと、
  ⚠ 「画面が自分の `_ready()` で消すもの」（⚠ 右上の通貨）が**本番と違う状態で写る**
- ⚠ 撮るのは**役割ごとに1枚**（⚠ ①ランの中で判断する ②戦う ③拠点の入口 ④育てる ⑤回す）。
  ⚠ 同じ役割を2枚撮らない（⚠ `floor_map` と `dungeon_map` だけは**器が別**なので両方撮る）
- ⚠⚠ **窓も撮れる（2026-09-22 から・いま 18枚）。** ⚠ 撮り方は2通り。⚠ **どちらも本番の口を通す**
  - ⚠ `SceneManager` で開くもの（⚠ レリック選択・商人・わかれ道）＝ ⚠ 行に `prepare` を足すだけ
  - ⚠ 重ねるもの（⚠ 拾いもの・戦闘の結果・共通モーダル）＝ ⚠ 行に `after` を書く
    （⚠ `ShotTaker._after()` に手が1つずつ。⚠ **窓のシーンを単体で開かない**＝後ろが無く幕が透けない）
- ⚠⚠ **`Modal.confirm()` は撮るのに使えない**（⚠ coroutine。⚠ 直に呼ぶとパースエラー、
  ⚠ `Callable` 越しでも実行時に赤、⚠ `await` すると押されるまで戻らない）。⚠ 器を返す `Modal._enqueue()` を呼ぶ
- ⚠ **まだ撮れない窓**：⚠ 通路のできごと（⚠ 本番は `await` で閉じるまで待つ形）／ ⚠ シナリオ側の商人（`floor_shop`）
- ⚠ **色・活性・手応え・遷移は、絵でも分からない。** ⚠ それは人間の「見る回」へ

### ⚠⚠ 実機確認は「見る回」にまとめる（2026-09-21・人間の決定）

**人間の言葉：「⚠ 実機確認が重いが重要でもある　⚠ だから頻度を減らして一気にチェックしたい」。**

- ⚠⚠ **溜め先は `docs/HUMAN_CHECK.md` の1枚**（2026-09-21 に新設）。⚠ 回の節に書かない（⚠ 散って数えられなくなる）
- ⚠⚠ **1タスクごとに人間へ渡さない。** ⚠ **「絵で判断できなかったもの」が3件たまったら「見る回」を置く**
- ⚠ **回数では数えない。** ⚠ 数えるのは**中身**（⚠ 絵で潰せた回は人間の番が来ない）
...
| **画面（在るか・並び・大きさ・形）** | ⚠⚠ **設計役**（`scenario=shot` の絵を読む・2026-09-21〜） | 無し |
| **画面（色・活性・手応え・遷移・気づけるか）** | **人間だけ** | **操作して見る**（⚠ 「見る回」にまとめる） |

**§0 事前チェックは、人間に渡す前に設計役が必ず1回回す。**
ロード時の赤・黄・parse error はここで全部潰してから渡すこと。
**通っていないものを人間に渡さない。**

⚠⚠ **2026-09-21 に変わった：画面の絵は取れる**（⚠ `--headless` を外して `scenario=shot`。⚠ 上の「画面は撮れる」）。
⚠ **ヘッドレスでは取れない**（⚠ 描画がダミー。⚠ `--write-movie` は `Parameter "t" is null` / アクセス違反で落ちる）。
⚠⚠ **「画面」の章がまるごと設計役に移るわけではない。**
⚠ **絵で分かるもの**（⚠ 在るか・並び・大きさ・形）は設計役が潰す。
⚠ **絵でも分からないもの**（⚠ 色・活性・手応え・遷移・気づけるか）は**人間の「見る回」**へ送る。
...
### ランダムに作られるものを歩く検査は、結果が揺れる（2026-09-20 に追加）

`scenario=dungeon` の「宝箱を持ち帰る」検査は、**歩いた道にたまたま罠（鞄）があると赤になる**
（鞄の中身を落とすため）。マップは毎回ランダムなので、**同じコードでも通ったり落ちたりする。**

- **赤が出たら、まず「その回のログで何が起きたか」を読む。** コードを疑う前に、生成の揺れを疑う
- 揺れる検査は「何が起きたか」を必ず print してから赤にする（⚠ 読めないと毎回コードを疑うことになる）
- 直すなら検査側（⚠ 落とされたぶんを数える／罠の無い道を選ぶ）。**本番の生成を検査のために変えない**
