...
### 0-0. ⚠ 前の回の人間の宿題（**残り3項目・着手前に片付ける**）

⚠ **`ja.csv` の再インポートは済んでいる。⚠ 画面（`EXEC_WORKSHOP_REVIVE.md` §5-C）も、⚠ 「製作が完成するのを待つ」に依存する項目以外は全部通った**（2026-08-25・人間の報告）。

⚠ **待てなかったので `F4` に「製作をすぐ完了させる」を足した**（`tests/debug_overlay.gd`。⚠ **`F4` の情報欄に `製作 N / M本（完成 K）` の1行も出る**）。⚠ **残っているのはこれを使う3項目だけ：**

1. ⚠ **作業場で「作る」→ `F4` →「製作をすぐ完了させる」→「受け取る」**
   ⚠ **倉庫の持ち物に装飾が1個増えているか**（`EXEC_WORKSHOP_REVIVE` §5-C の C-9 / C-11）
2. ⚠ **同じレシピを2回以上回して、⚠ 出るものが毎回同じではないか**（C-12）
3. ⚠ **そのあと1回セーブしてもらう**（⚠ **`save_slot_0.json` に `"duration_sec": 1440.0` のような `.0` が無いことを設計役が読む＝§5-B の B-8**）
