...
**ズレが31回起きている。** ⚠ **`grep` で関数の中身を見てから判断する。⚠ 違っていたら報告する（勝手に直さない）。**

⚠ **未報告のズレは 0件。⚠ 次に見つけたものは 32 番。**
⚠ **ルーンの回（段階8）では1件も見つからなかった。** ⚠ **段階9で3件見つかった**（⚠ **どれも `GAME_DESIGN` / コメント側を直していない**）：

| # | ズレ | 直すなら |
|---|---|---|
| ⚠ **29** | ⚠ **`GAME_DESIGN` 9-5 は「装備 → 育成」の順だが、⚠ 装備画面へは育成画面からしか入れない**（`training_screen.gd:292`） | ⚠ **9-5 の表**（⚠ **決定4「同時に開く」で回避してある**） |
| ⚠ **30** | ⚠ **9-5 の10段のうち、画面として閉じられるのは6つだけ。** ⚠ **装飾（#5）とルーン（#10）は装備画面の中の行 ／ 拠点（#8）はハブ ／ 倉庫はどの段にも無い** | ⚠ **9-5 の表**（⚠ **決定5・6 で埋めてある**） |
| ⚠ **31** | ⚠ **`master_data_loader.gd` のコメントは「パッシブは `characters/<id>/passives.json` に置く」と書いているが、⚠ 実際は `skills.json` に `activation: "passive"` で同居している** | ⚠ **コメント側か、⚠ ファイルを分けるか。⚠ このタスクの §1-2 の質問3** |
...
⚠ **2026-08-24に踏んだ**：⚠ **`scenario=parts` に「IDと欄の綴りが一致しないもの N 件」という集計が在り、⚠ ルーン25件を足したら25件とも「一致しない」に数えられた**（⚠ **ルーンには軸が無いので欄からIDを組み立てられない**）。
→ ⚠ **既存の器の型を見るのと同じで、⚠ 既存の「数えている行」も見ること。⚠ 数字が静かに嘘になる。**

### ⚠ `.tres` を直す作業は、結果が数字で読める形にして渡す

⚠ **2026-08-24に踏んだ**：⚠ **`initial_state_config.tres` の編集は設計役にはできない**（`CLAUDE.md`）。⚠ **そのままだと「やったつもり」で先に進める。**
→ ⚠ **`scenario=unlock` に「`.tres` は直っている／まだ直っていない」の1行を出すようにした。⚠ 人間の作業を完了条件に書くときは、⚠ 済んだかどうかを設計役が観測できる合図も一緒に作ること。**

### ⚠ 「赤が1本出るのが正解」を作るときは、平常値をドキュメントに書く

⚠ **`E125` が本当に出るかを確かめるため、⚠ `scenario=unlock` で意図的に赤を1本出している**（⚠ **`drops` が黄を1本多く出すのと同じ形**）。
→ ⚠ **`unlock` は赤1本が平常。⚠ 2本以上出たら本物。** ⚠ **平常値を書かないと、⚠ 次の回に「赤が出ている」で止まる。**
