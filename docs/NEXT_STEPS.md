...
- ⚠ **`PLAN_SCENARIO_MAP.md` §3-2 の矛盾**：⚠ **「合流なしの2分岐」と「ボスは固定位置」は両立しない**（⚠ **次のタスクの本題**）
...
- ⚠ **`PLAN_SCENARIO_MAP.md` §9 の見積りが「1タスクに収まらない」**（⚠ **新規シーン4本・新規マスター3〜4本・新しい状態キー1本・既存の改修5本・`ja.csv` 30〜50行**）
- ⚠ **段階の割り方を決めないまま `EXEC` を書くと、⚠ 1本目が肥大して必ず溶ける**
- ⚠ **割り方は §3-2（合流の有無）と §3-3（段数）が決まらないと確定できない。⚠ この2件だけ人間の判断が要る**

### 1-1. ⚠ 何をするか

| | 誰が | 何を |
|---|---|---|
| **1** | ⚠ **人間** | ⚠ **`PLAN_SCENARIO_MAP.md` §3-2 の矛盾を裁く**（⚠ **設計役の推奨は (a) 最終段だけ合流**） |
| **2** | ⚠ **人間** | ⚠ **§3-3 の段数を決める**（⚠ **推奨5段＝31ノード生成・歩くのは5マス**） |
| **3** | ⚠ **設計役** | ⚠ **§10-1 の残り2件を決める**（⚠ **段階の割り方 ／ `stage_dbg_*` 6本の扱い**） |
| **4** | ⚠ **設計役** | ⚠ **`PLAN_SCENARIO_MAP.md` §3 と §10-1 を確定に書き換える** |
| **5** | ⚠ **設計役** | ⚠ **`PLAN_IMPLEMENTATION.md` 3章に段階14 を足す**（⚠ **割った数だけ行を足す**） |
| **6** | ⚠ **設計役** | ⚠ **`EXEC_SCENARIO_MAP_*.md` の1本目を書く**（⚠ **完了条件は「ログ」「ファイル」「画面」の3つに分ける**） |

### 1-2. ⚠ 予想できている落ち（**先に潰すこと**）

- ⚠ **1本目に「マップ画面」を持ってこない。⚠ 画面は人間しか見られないので、⚠ 検証が全部人間側に寄って詰まる**（⚠ **推奨：1本目は `stages.json` を5フロアにする＋`unlocks` の付け替え＝`scenario=unlock` と `scenario=economy` で取れる範囲**）
- ⚠ **`unlocks` を触る回では `scenario=unlock` の赤の平常値が **1本**（`E125`）であることを先に確かめる**
- ⚠ **ハズレ枠を廃止する回では `scenario=economy` を必ず回し、⚠ 前後の数字を EXEC に並べる**（⚠ **1周あたりの宝箱数が全部動く＝`PLAN_SCENARIO_MAP.md` §4-3**）
- ⚠ **`chest_id` を足す回では `scenario=drops` も回す**（⚠ **`drops` は `chests.json` しか見ていない＝宿題54。⚠ 逆にここは効く**）
- ⚠ **新しい状態キーを足したら `GameStateKeys` に定数を足し、⚠ `AGENTS.md` の表にも1行足す**
- ⚠ **`Balance` に欄を足すのはシーン編集＝人間の作業。⚠ 設計役は `.tscn` を触れない。⚠ 数値の置き場を決める段で必ず人間に渡すこと**
