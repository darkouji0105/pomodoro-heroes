...
### 0-0. ⚠ 前の回の人間の宿題（**全部終わっている**）

⚠ **`ja.csv` の再インポート・画面（`EXEC_WORKSHOP_REVIVE.md` §5-C の16項目）・セーブとも通った**（2026-08-25）。
⚠ **30分待てない項目のために `F4` に「製作をすぐ完了させる」を足してある**（⚠ **`F4` の情報欄に `製作 N / M本（完成 K）` の行も出る**）。

⚠ **セーブを読んだ結果、⚠ このタスクとは無関係の穴を1つ見つけた**（→ **§5 の宿題12**）：
⚠ **`load_state()` が `inventory` の `count` を `int()` に戻していない。⚠ セーブに `"count": 5.0` と書かれる**（⚠ **`materials` / `character_growth` / `crafting_queue` / `equipment_instances` は戻しているのに、⚠ ここだけ素通り**）。
⚠ **表示と判定は壊れない**（⚠ **読む側が全部 `int()` を通している**）。⚠ **直していない。⚠ 段階12 のついでに直すかは §1-2 で聞くこと。**
...
| ⚠ **`F4` の「製作をすぐ完了させる」** | ⚠ **`tests/debug_overlay.gd`。⚠ 製作中のものを全部 `completed` にする（⚠ 受け取りはしない）。⚠ このファイルで `_state` を直接書く唯一の例外＝`started_at` だけ** |
...
| **4** | ⚠ **宿題12（`inventory` の `count` が `5.0` で保存される）を、⚠ このタスクのついでに直すか** | ⚠ **直す。⚠ `load_state()` に4行足すだけで、⚠ 「1タスク=1つの通し」を破る大きさではない**（⚠ **ただし人間が「別の回にする」と言えばそれでよい**） |
...
| ⚠ **`F4` のデバッグパネル** | ⚠ **`tests/debug_overlay.gd`。⚠ 「ゴールド・ジェム・スタミナ」「素材を全種類」「消費アイテムを全種類」「装備を全種類 1個ずつ」「装飾を全種類」「研究を全部解放」「画面を全部解放」「製作をすぐ完了させる」「セーブする」**。⚠ **情報欄に `製作 N / M本（完成 K）` の行がある** |
...
12. ⚠ **`load_state()` が `inventory` の `count` を `int()` に戻していない**（⚠ **`game_manager.gd:5331` 付近。⚠ セーブに `"count": 5.0` と書かれる。⚠ 2026-08-25に実セーブで確認**）。⚠ **表示と判定は壊れない**（⚠ **読む側が全部 `int()` を通している。⚠ 一度でも増減すると int に戻る**）。⚠ **`CLAUDE.md` 3番の症状。⚠ 直すなら `materials` の3行の隣に4行足すだけ**
