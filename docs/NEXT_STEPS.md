...
⚠ **仕様は `GAME_DESIGN.md` 7-7 にまとまっている。⚠ 人間の決定3件は §1-2 にある。⚠ 聞き直さない。**
...
### 1-2. ⚠ 人間の決定（**2026-08-24。⚠ 聞き直さない**）

| # | 決定 |
|---|---|
| **1** | ⚠ **`parts` の型は変えない。** ⚠ **段階は `item_id` の連番で表す**（`part_rune_dash_1` 〜 `_5`。⚠ **宝石が既にその形＝`part_gem_hp_1` 〜 `_4` ＋ `part_tier` 欄**）。⚠ **要素は `{item_id, roll}` のままで、⚠ ルーンの `roll` は 0 固定**。⚠ **重ねる＝上位IDに置き換える** |
| **2** | ⚠ **ルーンは全種類入れる**：⚠ **バフ ／ デバフ ／ 移動 ／ 回復 ／ シールド**（⚠ **5種 × 5段階＝25件になる見込み。⚠ 宝石は既に36件並んでいるので件数としては前例の範囲**） |
| **3** | ⚠ **「ルーンのかけら」は今回作らない。** ⚠ **入手は `F4` のデバッグパネル**（⚠ **本番の入手経路＝ポモドーロのレア枠は後の回**） |

> ⚠ **決定1 の効き方**：⚠ **`PLAN_IMPLEMENTATION.md:51` が「`parts` の要素の型が変わる」を*唯一の破壊的変更*と呼んでいるが、⚠ この決定でそれが起きなくなった。⚠ 既存セーブも `_normalize_equipment_from_save()` もそのままでよい。**
> ⚠ **代償**：⚠ **段階ごとに別IDなので、リリース後にルーンのIDを改名できない**（`CLAUDE.md` 4番の一覧に加わる）。
