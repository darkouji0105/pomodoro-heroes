# 次にやること：**フロアの形を確定し、シナリオの作り替えを段階に割る**
...
⚠ **台帳は `docs/01_plan/PLAN_SCENARIO_MAP.md`。⚠ 先に読むこと。⚠ リソースの入口と出口は `docs/01_plan/PLAN_RESOURCE_FLOW.md`。**
⚠ **この回もコードを1行も書かない。⚠ 成果物は「フロアの形の確定」と「段階の割り方」と「EXEC の1本目」。**
⚠ **人間に決めてもらうのは2件だけ**（⚠ **`PLAN_SCENARIO_MAP.md` §3-2 の合流の有無と §3-3 の段数**）。⚠ **残りは設計役が決めて一覧で報告する。**

⚠ **`EXEC_PLAYTHROUGH.md`（通しで遊ぶ回）は保留のまま。⚠ ステージが作り替わるので、⚠ 作り替えたあとに回す。⚠ 消さないこと。**

---

## 0. ⚠ 前のタスク（**2026-08-25・シナリオの作り替えを「決める」回**）
...
## 1. ⚠ このタスク：**段階14-a（フロアの器）を作る**

⚠ **台帳は `docs/01_plan/PLAN_SCENARIO_MAP.md`。⚠ 決まっていることは全部あそこにある。⚠ ここに写さない。**
⚠ **やるべきこと全32件は §9-1、⚠ 6段階への割り方は §9-2。⚠ 14-a が受け持つのは 1・2・3・9・10・11・12・13・14・15・30 の11件。**
...
- ⚠ **`stage_dbg_*` 6本を巻き込まない**（⚠ **`_build_debug_stage_list()` は解放の連鎖に入れていない。⚠ 同じ扱いを保つ**）
...
⚠ **`PLAN_IMPLEMENTATION.md` 3章に段階14 を足す**（⚠ **段階単位の完了はあそこ1箇所で持つ**）。
