...
**残っているのはゴールドの出口。** ショップと作業場が無い。**次に作るのはショップ。** ゴールドは戦闘で増える一方で、使い道が1つも無い。
...
**ゴールドに出口が1つも無い。** 戦闘で増える一方で、使い道が無い。

`game_manager.gd`のショップ関連は3つとも空実装のまま。**研究と違い、読み取り側も書き込み側も何も無い。**

| 関数 | 状態 |
|---|---|
| `get_shop_lineup(shop_type)` | ラインナップが空なので常に`[]` |
| `purchase_shop_item(shop_type, slot_id)` | 空実装。常に`false` |
| `refresh_shop_if_needed(shop_type)` | 空実装。`print`のみ |

状態の構造（`DAILY_SHOP` / `WEEKLY_SHOP` / `MONTHLY_SHOP`、`SHOP_LINE_UP`、`SHOP_SLOT_ID`等）は`state_keys.gd`にそろっている。`Balance`に`ShopConfig`の口もある。

**研究で作った同期の型をそのまま使える**（`_sync_research_tree_from_master()`）。ただしショップは「マスターから流し込む」ではなく「抽選して組み立てる」ため、リフレッシュ時刻の判定が加わる。**ここが研究より難しい唯一の点。**

第2層（`PLAN_GUILD_SHOP.md`）は存在する。**実コードと突き合わせてからEXECを書く。** 育成でも研究でも、PLANは実コードを見ずに書かれていてズレていた。
...
| ショップの抽選テーブルとリフレッシュ | `PLAN_GUILD_SHOP.md` | **次のタスクで決める。** ゴールドの唯一の出口になる |
