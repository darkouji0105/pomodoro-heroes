...
- **宿題79** … `ScreenHeader.set_back_text()` がどこからも呼ばれていない（⚠ 戻るを「戻る」に統一した残骸）。消すか残すか
- **宿題80** … `char_debug_*` は `allocatable_stats` を持つのに `nodes.json` が無い（⚠ 割り振り画面で空の行が出る）
- **宿題81** … 戦闘の HUD がコンテナでないため `layout` で最小サイズが測れない（`0 x 0`）
- **宿題82** … PRE_PLAN_UI_DECISIONS.md §2 の最小サイズが古い（⚠ 正は `NEXT_STEPS.md` §0-UI-C-3）
