...
**着手の起点は `NEXT_STEPS.md`。** `PLAN_SKILL_TEMPLATE.md` 13章 ＋ 19章の段階3。

⚠ **段階3は大きすぎるので割る**（PLAN 19章）。**器を先に固めてから、購読と条件を乗せる。** 器が固まらないうちに乗せると両方直すことになる。

⚠ **「実行中のスキル層」と「状態の器」は別物**（PLAN 7-2）。**寿命の持ち主が違う。混ぜると、術者が死んだ瞬間に敵に付けたDoTが消える。** 段階2で作った `SkillRuntime` の中断（`cancel_for_user()`）がそのまま適用されてしまう形にしないこと。

⚠ **着手前に PLAN 20章「後から変えられない7つ」を必ず読む。** 特に **1番（状態は1ユニットに複数個持てる）** と **2番（寿命は秒数だけではない・`until:`）**。どちらも「上書き前提」「`duration_sec` 前提」で作ると**作り直し**になる。

⚠ **パッシブはこの後ろ。** 条件発動が要求する層は `buff` / `dot` が要求する層と**同一**で、2回に分けて作る意味が無い（PLAN 7-2・19章）。
