...
> - ✅ **2026-09-15：倉庫を OS の別窓で出し、窓をまたいで装備マスへドラッグできるようになった**（人間が実機で確認）。⚠ 出すのは拠点とギルド系だけ・右上の「倉庫」ボタンで出す出さない・ゲームの窓の右隣。⚠⚠ **倉庫はこの窓だけ**（⚠ ギルドのカードは消した）。⚠ **宝箱は拠点のバッジから開ける**（⚠ 倉庫の宝箱タブは消した）。⚠ 「ゲームの中」と設定画面は使い比べて消した。⚠ 正は `NEXT_STEPS.md` §0-UI-D
