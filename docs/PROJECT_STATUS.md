...
**ステータス9軸は入っていない。** 装備の第2弾と同じ会話でやる予定だったが、装備だけで終わった。`_stat_keys()`は`hp`/`atk`/`def`/`spd`の4本のまま（`game_manager.gd` 998行で確認）。**「9軸が入った前提」で書かれた記述が残っているので注意すること。**
...
**ステータス9軸**（`PLAN_STATS_AND_FORMULAS.md`）。**まだ1本も入っていない。** セーブ構造と`def`の計算式を触るため、着手するとバランスの測定値が無効になる。
...
**装備・鍛冶まで入り、コアループが全部繋がった。ここから先は「作る」ではなく「触る」。** 1周で何ゴールド・何素材入るかを一度も測っていない。詳細は`NEXT_STEPS.md`。

**計算道具（`balance_report.py`）は作っていない。** 一度作りかけたが、**リポジトリには存在しない**（`*.py`が0件）。**「作成済み」と書かれた過去の記述を信じないこと。** 道具は後回しにし、まず手で実測する。

**この順番には既知の矛盾がある。** 未確定表に「敵HP・スキル倍率のバランスは**ステータス9軸と式が入ってから**。`def`が除算になるためいまの数値は全部意味が変わる」と書いてある。**9軸を入れると今回の測定値の一部は無効になる。**

それでもいま測るのは、**測り方と記録の型を先に確立するため。** 逆算して直すのは、ゴールド収入・素材収入・ショップ価格・作業場のレート・鍛冶のコストのような**軸の本数に依存しない部分から**にすること。敵HPとスキル倍率は9軸のあとに回す。
...
1. **バランスの実測と、軸に依存しない数値の調整**（いまここ）
2. **ステータス9軸**（`PLAN_STATS_AND_FORMULAS.md`。**セーブ構造と`def`の式を触る。ここで敵HPを詰め直す**）
3. **パッシブ**（一本道・5レベルごと・20レベルおきに強い効果）
4. **宝石**（`tier`・ロール・振り直し）
5. **パーティ選択**
6. **ルーン**
7. **投射物**
...
| **等級の係数と上限** | **加算。** 基礎値 × `GRADE_STAT_RATIO`(0.25) × (等級-1)。**上限は3**（4〜10のコストは未定） |
| **鍛冶** | **装備画面に置く。待ち時間なし・失敗しない・素材だけ。** 作業場のレシピ形式は個体IDを渡せない |
| **枠（宝石・ルーン）** | **器だけ。** `parts: [null, null]`と`get_open_part_slot_count()`（等級5・10で開く計算）のみ。中身は未実装 |
...
| 等級4〜10のコスト | **係数は加算で確定**（乗算はインフレする）。上限は3のまま。**量と時間は実測から逆算する。バランスのタスクで確定させる** |
