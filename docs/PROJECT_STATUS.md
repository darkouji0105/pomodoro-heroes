...
**素材の使い道が1つ開いた。** 育成（レベルアップ）が入り、`training_material`を消費してキャラを強化 → 戦闘が楽になる、というループが閉じた。

**残っているのは、レベル上限の解放と、ゴールド・その他素材の出口。** 研究・ショップ・作業場が無い。**次に作るのは研究。** レベル上限を`base_level_cap = 10`の固定値で止めてあるため、そこを解放する側が要る。
...
- `MasterDataLoader`にキー一覧を返す関数が無い。`training_screen.gd`の`CHARACTER_IDS`が決め打ちになっている。キャラを増やすときはここも直す
...
**育成が上限で止まる。** `get_effective_level_cap()`は`Balance.character.base_level_cap`（現在10）と、解放済み`level_cap_unlock`ノードの`effect_value`合計を足して返す実装になっている。**研究ツリーが空なので、いまは全キャラがレベル10で頭打ち。**

研究側でノードを解放すれば、**育成画面もGameManagerも1行も変えずに上限が伸びる。** 接続はもう通っている。

第2層（`PLAN_GUILD_RESEARCH.md`）は存在する。**実コードと突き合わせてからEXECを書く。** 育成のときも、PLANが実コードを見ずに書かれていたためスコープごと書き直した。
...
| 研究ツリーのノード数・効果値 | `PLAN_GUILD_RESEARCH.md` | **次のタスクで決める。** 上限解放の刻み幅が育成の手触りを決める |
