...
45. ⚠ **Lv100 が遠すぎる**（⚠ **3キャラで `training_material_1` が 15,444 個＝`stage_2` を 3,861 周＝スタミナ 19,305＝集中 160.9 時間**）。⚠ **元は `character_config.tres` の `base_level_up_cost = 3` / `cost_growth_per_level = 1.0`＝人間しか直せない2行**
46. ⚠ **`training_material_2` / `_3` / `_4` に出口が1つも無い**（⚠ **レベルアップが使うのは `level_up_material_id` の1件だけ。⚠ 段階ごとに素材を変える器が無い**）。⚠ **`stage_3` は `training_material_1` を1個も落とさないので、⚠ 進むほど育成が止まる＝`GAME_DESIGN` 4-1 と逆向き**
47. ⚠ **`decor_material_4` に出口が1つも無い**（⚠ **装飾の段階が4止まり＝`max_part_tier`。⚠ 作業場のレシピも `_1..3` だけ**）
