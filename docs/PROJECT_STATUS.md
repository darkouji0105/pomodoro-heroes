...
65. ⚠ **`resources/balance/` のフォルダ整理が残っている**（⚠ **段階14-i-2。⚠ 人間の決定：`configs/` に `.gd` と `.tres` をペアでまとめる**）。⚠ **`balance.tscn` は13本すべてを `path=` で参照し、⚠ うち3本（`shop` / `research` / `workshop`）は `uid` を持たない＝移動すると `Balance.X` が黙って null になる**。⚠ **手順と落ちは `NEXT_STEPS` §0-C-4**
