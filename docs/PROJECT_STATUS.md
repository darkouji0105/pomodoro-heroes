...
63. ~~⚠ **道中の戦闘ノードに報酬が1つも無い**~~ ✅ **閉じた**（2026-08-28・段階14-i）。⚠ **ボスの `gold` を半分にして、⚠ 残り半分を `node_rewards.gold`（新設）として道中の戦闘へ配り直した。⚠ 1周の期待ゴールドは据え置き（幅が出るようになった）**。⚠ **判定は `get_floor_node_rewards()` の1本。⚠ 全文は `PLAN_SCENARIO_MAP.md` §10-2-B**。⚠ **以下は当時の記述**：
   - ⚠ **道中の戦闘ノードに報酬が1つも無い**（⚠ **`battle_controller.gd:1652` の `if (not in_floor_run) or is_boss:`＝段階14-c の意図どおり**）。⚠ **一方で宝箱は「移動」に紐づくのでどのルートでも同じ数。⚠ つまり戦闘ノードは純粋なコストで、⚠ 避けるのが常に最適になる。⚠ 段階14-h では `battle` の重みを底上げして「避けきれない」形にしただけで、⚠ 報酬は付けていない**（⚠ **`.json` では書けない。⚠ `battle_controller` の分岐とノードごとの報酬が要る**）。⚠ **人間の決定（2026-08-28）で段階を分けた＝`PLAN_SCENARIO_MAP.md` §10-2-B**
64. ⚠ **休憩ノードの回復量を割合にできない**（⚠ **`rest_at_node()` は `Balance` を1行も読まず常に満タン**）。⚠ **`floor_rest_full_heal` という欄はあったが誰も読んでおらず、⚠ 2026-08-28 に消した（ズレ46）**。⚠ **やるなら `FloorConfig` に `rest_heal_pct`（`shop_heal_pct` と同じ形）を足し、⚠ `rest_at_node()` が `hp_carry` を書き換える形にする。⚠ 実装せずに欄だけ足さないこと＝同じ死に欄をもう1本作ることになる**
65. ~~⚠ **`resources/balance/` のフォルダ整理**~~ ✅ **済んだ**（2026-08-28・段階14-i-2）。⚠ **`configs/` 44件 ／ `master/` 19件。⚠ 直下は空**。⚠ **`AGENTS.md` も人間の許可を得て更新した**。⚠ **踏んだ落ちは `NEXT_STEPS` §0-C-4**
66. ⚠ **`tests/test_common_infra.gd` がパースエラーで読めない**（⚠ **2026-08-28に発見。⚠ `--import` を回して初めて見えた＝通常のシナリオは到達したものしか読まない**）。⚠ **`git diff` で確認済み＝段階14-i では1文字も触っていない。⚠ 元から壊れていた**。⚠ **宿題7「`tests/` の既存9件の棚卸し」の実体の1つ**
