...
> ⚠⚠ **2026-09-20 時点（ここが正）。** ⚠ この下の本文は 2026-08 時点の記録で、**「次はパッシブ」などは古い**。
>
> - ⚠ **UI の仕上げの途中。** 人間がモックを渡し、設計役が実装して、人間が実機で見る回が続いている（09-07〜09-14）
> - ⚠ **レベルの役割転換は3つとも入っている**：割り振り（ステータスノード）／スキル選択（2枠）／**パッシブ（総ポイント 20/40/60/80/100 で自動解放）**
> - ⚠ **難ダンジョンは段階20まで入っている**（UI 11件が残り）
> - ⚠⚠ **2026-09-21 に台帳を3つに分けた**（⚠ 人間の決定）：
>   ⚠ **いま効いている値の正は `docs/DECISIONS.md`**（⚠ 1決定＝1行）／
>   ⚠ **いまの議題は `NEXT_STEPS.md`**（⚠ 直近3節だけ。⚠ 前の回は `docs/03_log/NEXT_STEPS_2026-09.md` と `..._2026-08.md`）／
>   ⚠ **`PLAN_◯◯.md` はとっかかり**（⚠ 経緯。⚠ 値は古いことがある）
> - ⚠ 平常値・最小サイズは `NEXT_STEPS.md` の直近の節（⚠ 古いものは `docs/03_log/` の月ごとの記録へ移した）
> - ✅ **GAME_DESIGN.md のパッシブの記述は 2026-09-14 に決定へ合わせた**（5-2・5-4・変更表）
> - ✅ **2026-09-15：倉庫を OS の別窓で出し、窓をまたいで装備マスへドラッグできるようになった**（人間が実機で確認）。⚠ 出すのは拠点とギルド系だけ・右上の「倉庫」ボタンで出す出さない・ゲームの窓の右隣。⚠⚠ **倉庫はこの窓だけ**（⚠ ギルドのカードは消した）。⚠ **宝箱は拠点のバッジから開ける**（⚠ 倉庫の宝箱タブは消した）。⚠ 「ゲームの中」と設定画面は使い比べて消した。
> - ✅ **2026-09-16：装備画面を左右2列にした**（左＝ステータス／右＝装備関連。⚠ 実機ではまだ見ていない）。⚠ `Modal` の穴（消えた呼び出し元）も閉じた。⚠ 正は `NEXT_STEPS.md` §0-UI-D
> - ✅ **2026-09-16：戦闘画面の骨格を描き直した**（HUD をコンテナ化・ヘッダー「層 波 n/m」・中央の縦線・UnitView 74px・HPとシールドを1本・下部パネル3分割）。⚠ スキルの3種・チャージバー・結果窓・チップ6色は次の回。⚠ 実機ではまだ見ていない。⚠ 正は `NEXT_STEPS.md` §0-UI-E
> - ✅ **2026-09-16：戦闘中は右上の資源を出さない／スキルボタンを絵のマス（3種を動きの向きで描き分け）にした**。⚠ 中央のチャージバーは次。⚠ 実機ではまだ見ていない。⚠ 正は `NEXT_STEPS.md` §0-UI-F
> - ✅ **2026-09-17：戦闘の結果窓／状態異常のチップの色**。⚠ 正は `NEXT_STEPS.md` §0-UI-G・§0-UI-H
> - ✅ **2026-09-18：戦闘の残り一気に／窓の縁と題の帯を全モーダルへ／敵の行動予告の SP／雑魚のスキル・宝箱の絵と色**。⚠ 正は §0-UI-I 〜 §0-UI-K
> - ✅ **2026-09-18：鞄をシナリオにも**（⚠ 難ダンジョンと同じ形。⚠ 宝箱は「宝箱のまま鞄へ」運び、中身は拠点で引く）。⚠ 正は §0-UI-L
> - ✅ **2026-09-19：難ダンジョンとシナリオの UI を共通に**（⚠ マップの部品 `RunMapView` ／ レリック選択は1枚 `run_relic_select`）。⚠ 正は §0-UI-M
> - ✅ **2026-09-19〜20：難ダンジョンの画面をモック v2 の形に**（⚠ 真鍮の札のマス・手描きの通路・層の目盛り・区画の切れ目・たいまつの明かり・拾いものの窓・吹き出し・HPバー・遺物片のアイコン）。⚠ 正は §0-UI-N
> - ✅ **2026-09-21：⚠ 溜まっていた「実機で見る」を全部片付けた**（⚠ 人間「⚠ 全部試した」「⚠ 全部試して正常だった」）。
>   ⚠⚠ **未確認は `docs/HUMAN_CHECK.md` で数える。⚠ いま 0 件**
> - ✅ **2026-09-21：⚠ 進め方を作り直した**（⚠ 台帳3つ＋見る回＋絵を撮る＋2者の体制）。⚠ 正は §0-UI-Q / §0-UI-R / §0-UI-S
> - ✅ **2026-09-21：⚠ 共通モーダル（決定39）／ ⚠ ボスの後のわかれ道（決定48）**
> - ⚠ 2026-09-20：小さい掃除（⚠ 使われない翻訳キー4本 ／ ⚠ `FLOOR_RUN.consumables` の欄）を消した
...
- ~~受け取り報告の文言。宝箱0個のとき「宝箱を0個」と出る~~ **✅ 既に直っている**（2026-09-14 に確認。`base_screen.gd:193-200` が数で分岐）
...
- ~~`MasterDataLoader`にキャラのキー一覧を返す関数が無い~~ **⚠ 誤り**（2026-09-11 に発見）。`get_all_characters()` は在る
...
- ~~**`PomodoroConfig.reflection_time_limit_sec`と`reflection_min_chars`が使われていない。**~~ **✅ 両方とも直った**（文字数は 2026-09-10、秒数は `633be7e`・2026-09-12）。⚠ 以下は当時の記録： `pomodoro.gd`が`const REFLECTION_TIME_LIMIT_SEC: float = 120.0`をハードコードしている。**数値管理ルール違反。** 値が一致しているため実害は出ていない（`EXEC_SOUND.md` §11）
...
- ~~**宿題78** … GAME_DESIGN.md のパッシブの記述を「総ポイント 20/40/60/80/100 で自動解放」に合わせる~~ ✅ **2026-09-14 に直した**（人間の許可。5-2・5-4・変更表の4箇所）
- ~~**宿題79** … `ScreenHeader.set_back_text()` がどこからも呼ばれていない~~ ✅ **2026-09-22 に消した**（⚠ 呼び出し0件。⚠ 「戻る」に統一した決定で用が無くなった。⚠ 空のキーで潰れる守りは残した）
- ~~**宿題80** … `char_debug_*` は `allocatable_stats` を持つのに `nodes.json` が無い~~ ✅ **2026-09-22 に置き場を出す形にした**（⚠ 段が1つも無いキャラは `EmptyState` 1行。⚠ 空のカードを軸の数だけ並べない）
- ~~**宿題81** … 戦闘の HUD がコンテナでないため `layout` で最小サイズが測れない（`0 x 0`）~~ ✅ **2026-09-16 に解消済み**（⚠ HUD をコンテナ化。⚠ `scenario=shot` の `measure` でも取れている。⚠ **台帳が追いついていなかった**）
- ~~**宿題82** … PRE_PLAN_UI_DECISIONS.md §2 の最小サイズが古い~~ ✅ **2026-09-22 に断り書きを足した**（⚠ 正は `docs/DECISIONS.md`。⚠ 経緯は `docs/03_log/NEXT_STEPS_2026-09.md` §0-UI-C-3）
...
49. ~~⚠ **`GAME_DESIGN` 4-1 の「周回ステージ・スキップ周回」が実装に1件も無い**~~ ✅ **閉じた**（2026-08-25・段階14-f）。⚠ **以下は当時の記述**：⚠ **`GAME_DESIGN` 4-1 の「周回ステージ（素材系／装飾系）」と「スキップ周回」が実装に1件も無い**（⚠ **`stages.json` はシナリオ3本だけ**）。⚠ **段階12 は「作らない・測るだけ」にしたので未決のまま**
...
58. ~~⚠ **`stage_2` をクリアするまでスタミナの入口が1つも無い**~~ ✅ **閉じた**（2026-08-25・段階14-a）。⚠ **`unlocks` でポモドーロを `floor_1` へ前倒しした**。⚠ **以下は当時の記述**：⚠ **`stage_2` をクリアするまでスタミナの入口が1つも無い**（⚠ **ポモドーロが開くのが `stage_2` のクリアのため**）。⚠ **初期スタミナ 20 ＝ **4周**で `stage_1` と `stage_2` を抜ける必要がある**（⚠ **負けた周は `refund_stamina` で戻るので、⚠ 減るのは勝った周だけ**）。⚠ **通しで本当に詰まるかは `EXEC_PLAYTHROUGH` §5-C の C-2〜C-4 で観測する**
...
62. ⚠ **下に既にある**（⚠ **2026-08-28 に「落ちていない」と誤認して二重に書いた。⚠ ズレ45 は誤り＝元から落ちていた。⚠ 番号の若い側を消してこちらに寄せた**）
...
67. ⚠⚠ **スタミナを1も払わずに宝箱を集められる**（⚠ **2026-08-31に発見。⚠ `docs/FLOOR_GAMEPLAY_CURRENT.md` §6 論点6 に全文**）。⚠ **`start_floor()` はスタミナを払わず、⚠ 消費するのは「ボスに勝ったとき」だけ**（`battle_controller.gd:1671` の `if (not in_floor_run) or is_boss:`）。⚠ **負けても降りても減らない**（`:1851`＝「負けてもノーリスク」の決定）。⚠ **一方で拾った宝箱は `pending_chests`（トップレベル）に入るため、⚠ `abandon_floor()` しても残る**。→ ⚠ **「入る→6回歩く→降りる」を繰り返すとスタミナ0で宝箱が無限に増える**。⚠ **`scenario=economy` の集中時間の表（42.3〜15.6 時間）は「1周5スタミナ」を前提にしているので、⚠ 前提ごと崩れている**。⚠ **直すと 2026-08-25 の決定（負けてもノーリスク）を覆すことになるので、⚠ 人間が裁くまで実装しない**
68. ⚠ **仮アセットのアイコンが「行の先頭に40px」を足したが、⚠ 横幅への影響を測れていない**（⚠ **2026-08-31**）。⚠ **倉庫・ショップ・装備の一覧はどれも `ScrollContainer` の中で、⚠ `scenario=layout` は中を測れない**（⚠ **既知の制約＝`NEXT_STEPS` §3-1**）。⚠ **人間が実機で見る項目**
...
| **「`workshop` の赤2本は前からある」と平常値に書いていたが、わざと壊す検査だった**（2026-09-14） | 直すべき残骸だと読める書き方になっていた。**赤が出るのが正しい検査は「わざと」と明記する**（`NEXT_STEPS.md` §0-UI-C-2） |
| **`ScreenHeader` の戻るが空になり、2画面で潰れていた**（2026-09-14・人間が実機で発見） | 副題を入れると `_refresh()` が戻るの文字を `tr("")` で上書きしていた。**ヘッドレスでは絵が取れないので、`layout` に「戻るが空なら赤」を足した** |
| **台帳の「総ポイント＝レベル-1」を信じて人間に「最大99pt」と伝え、誤った前提で選ばせた**（2026-09-14） | 実際は最大レベルだけ +1。**選択肢に数字を書くときは、その数字を出している関数を先に読む** |
| **未使用キーの判定で、定数の接頭辞（`CATEGORY_KEY_PREFIX`）で組まれるキーを未使用と誤判定した**（2026-09-14） | `"ui_xxx_" +` だけ探していた。**`const ... = "ui_..._"` も探してから消す** |
...
- **台帳を実コードに合わせた時点（2026-09-14・人間の決定「A. 台帳を実コードに合わせる」）**：
  - 「現在地」の冒頭に 2026-09-14 時点の要約を足した（⚠ 本文は当時の記録として残す）
  - 溜まっている宿題のうち**解消済み3件・誤り1件**に印を付けた。⚠ 新しい宿題 78〜82 を足した
  - 事故一覧に4件足した（わざとの赤の書き方／戻るが空／99pt の誤った前提／未使用キーの誤判定）
  - ⚠ **正は `NEXT_STEPS.md` §0-UI-C**（平常値・最小サイズ・決定・残り・ズレ14件）
