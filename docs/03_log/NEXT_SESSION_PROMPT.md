...
- ⚠ `DungeonEdgeLines` を `scenes/ui/components/` へ（⚠ 使うのは `RunMapView` だけ。⚠ uid の手当てが要る）
- ⚠ 使われなくなったキー：`ui_relic_give` ／ `ui_dungeon_shop_torch_now` ／ ⚠ 罠（鞄）の2キー（`ui_dungeon_edge_event_trap_bag` / `ui_dungeon_edge_title_trap_bag`）
- ⚠ マスの文言が2通り：シナリオ「たたかう」／難ダンジョン「戦闘」
- ⚠ `DungeonChest` の名前（⚠ シナリオでも使う窓）
- ⚠ `FLOOR_RUN.consumables` は誰も読み書きしない欄 ／ ⚠ `PROJECT_STATUS.md` の「現在地」が 09-16 で止まっている
...
⚠ 拾いもの **620 x 316** ／ ⚠ レリック選択 **712 x 204** ／ ⚠ 商人 **888 x 320** ／ ⚠ 戦闘の HUD 581 x 145。
