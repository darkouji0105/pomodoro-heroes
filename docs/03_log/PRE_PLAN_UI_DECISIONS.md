...
> ⚠⚠ **2026-09-22（宿題82）：⚠ このファイルは「当時の記録」。⚠ 値は追いかけていない。**
> ⚠ **いま効いている値の正は `docs/DECISIONS.md`**（⚠ 1決定＝1行）。
> ⚠ とくに **§2 の最小サイズは古い**。⚠ 当時の経緯は `docs/03_log/NEXT_STEPS_2026-09.md` §0-UI-C-3
> （⚠ `NEXT_STEPS.md` から月ごとに移した先）。⚠ **いまの寸法は `scenario=shot` の `measure` で取り直せる。**
