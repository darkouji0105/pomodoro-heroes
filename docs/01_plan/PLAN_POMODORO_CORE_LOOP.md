...
2. **前回の端数（`potion_focus_remainder`）を足してから**、ポーションの獲得数を算出する
   - `合計分 = 集計した作業分 + potion_focus_remainder`
   - `獲得数 = 合計分 / 25`（切り捨て）
   - `新しい端数 = 合計分 % 25` を`potion_focus_remainder`に保存する
   - **加護による差はつけない。** スタミナは「その日に遊べる時間」に直結するため、加護で差をつけると長く働かないと遊べない設計になる（DATA_SCHEMA 1章）
3. ポーションを`GameManager.add_to_inventory()`でインベントリへ追加する
   - スタミナを直接付与しない
   - gold・素材も当面0とする
...
5. `GameManager.apply_pomodoro_rewards()`で`total_pomodoro_completed`の加算と`SignalBus.pomodoro_session_completed`の発火を行う
6. `SceneManager.change_scene()`で拠点画面へ戻る

拠点に戻ると、ポーションの所持数が増え、宝箱を獲得していれば`ChestBadge`が点灯している状態になる。**これがコアループの手応えとして機能する。**

- 2セットだけで切り上げても、そこまでのポーションと、跨いだしきい値の宝箱は確実に受け取れる
- 端数（25分に満たなかったぶん）は次回へ持ち越す。切り捨てで損する感覚をなくすため
- `unclaimed_chests`は受け取り終えるまで空にしない

### 6-2-1. ポーションの使用

- ポーションは`GameManager`の関数（例：`use_stamina_potion()`）で1個消費し、スタミナを回復する
- **回復は`max`を超えてよい**（例：60のときに飲むと110になる）。`max`は自然回復の上限であり、自分で貯めたポーションを飲んだぶんは超えてよい。上限で切り捨てると「満タン近くで飲むと損」が頻繁に起きるため
- 自然回復やその他の`add_stamina()`は従来どおり`max`で切り捨てる
- **倉庫画面がまだ無いため、当面は拠点画面の下部にポーションの所持数と「使う」ボタンを置く。** 倉庫が実装されたらそちらへ移す
...
- ポーション獲得レート（25分で1個）と回復量（50）の調整 → 実際に遊んでから見直す
...
- [ ] 拠点画面のポーション所持数が獲得分だけ増えている
- [ ] ポーションを使うとスタミナが回復し、`max`を超えても切り捨てられない
- [ ] 25分に満たない端数が次回のセッションへ持ち越される
...
- **改訂（加護の仕組みを倍率から宝箱へ変更）**：
  ...
  - **報酬をスタミナ直接付与からスタミナポーションに変更**（作業25分で1個・加護によらず一律）。端数は`potion_focus_remainder`として次回へ持ち越す。ポーションは`max`を超えて回復できる
