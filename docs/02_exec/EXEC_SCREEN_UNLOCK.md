...
| # | 項目 | 結果 |
|---|---|---|
| 1 | 新規開始で3つだけ | ✅ **`["adventure_select", "settings", "scenario"]`**（⚠ **人間が `.tres` を直したあとに取り直した。§14**） |
| 2 | `stage_1` で4つ増える | ✅ **`+["guild", "equipment", "training", "warehouse"]`** |
| 3 | `stage_2` で2つ増える | ✅ **`+["pomodoro", "decoration"]`** |
...
## 13. ⚠ 追加で直したもの — **「セーブを消しても消えない」**（2026-08-24・**人間が実機で発見**）

### 13-1. ⚠ 何が起きていたか

⚠ **セーブファイルは正しく消えていた。⚠ 残っていたのはメモリ上の状態。**

| | 事実 |
|---|---|
| ⚠ **`SaveManager.delete_save()` は正常** | ⚠ **`DirAccess.remove_absolute()` でファイルを消している。⚠ ここにバグは無い** |
| ⚠ **`GameManager._state` を作るのは2箇所だけ** | ⚠ **`_ready()`（起動時1回）と `load_state()`（つづきから）。⚠ リセットする口が1つも無かった** |
| ⚠ **再現する手順** | ⚠ **つづきから → 遊ぶ → タイトルへ戻る → セーブを削除 → 最初から**。⚠ **遊んだ状態がそのまま出る** |
| ⚠ **見え方** | ⚠ **「消したのに消えていない」。⚠ ゲームを終了して再起動すると消えているので、⚠ 「消えるときと消えないときがある」に見える** |
| ⚠ **同じ穴のもう1本** | ⚠ **版が違ってロードに失敗した枝も「新規開始として続行する」と書いてあるが、⚠ そこも状態を作り直していなかった** |

### 13-2. ⚠ 直し方

| ファイル | 中身 |
|---|---|
| `autoload/game_manager.gd` | ⚠ **`_ready()` の中身を `_build_new_game_state(caller)` に切り出し、⚠ 公開の `reset_to_new_game()` を新設**（⚠ **2本に分けて書かない。⚠ 片方だけ直すと「起動直後は正しいが最初からを押すと壊れている」になる**） |
| `scenes/title/title_screen.gd` | ⚠ **`_on_start_pressed()` で「新規開始か」を1つの変数に集め、⚠ そのときだけ `reset_to_new_game()` を呼ぶ**（⚠ **ロード失敗の枝も通る**） |
| `tests/debug_boot.gd` | ⚠ **`scenario=unlock` に (d) を1本足した** |

- ⚠ **`_validate_balance_item_refs()`（E121）は `_ready()` に残した**（⚠ **マスターは変わらないので、⚠ 最初からのたびに走らせない**）
- ⚠ **「新規開始」を決めている場所は `title_screen._on_start_pressed()` の1箇所だけ。⚠ 2本目を作らないこと**

### 13-3. ⚠ 実測（`scenario=unlock` の (d)）

```
  遊んだ状態  gold=100099 開いている画面=12件
[GameManager] reset_to_new_game() — initializing from Balance.initial_state
  最初から後  gold=100 開いている画面=5件
```

- ✅ **`gold` が `initial_state_config.tres` の 100 に戻った**
- ✅ **開いている画面が 12件 → 5件**（⚠ **クリア済みが消え、⚠ `.tres` の初期値だけになった**）
- ⚠ **セーブファイルは触っていない**（⚠ **`debug_boot` は保存も削除もしない**）

### 13-4. ⚠ この直しで §7-A が変わった

⚠ **`initial_state_config.tres` を直したあと、⚠ ゲームを再起動しなくても「セーブを削除 → 最初から」で確かめられる。**
⚠ **これが無いと §7-B の12項目は「毎回ゲームを再起動する」必要があった。**

### 13-5. ⚠ 黄の平常値が変わった（**`ja.csv` の再インポート済み**）

| | 前 | いま |
|---|---|---|
| 平常の黄 | ⚠ **16本** | ✅ **1本**（`skill_dbg_dot_odd`。⚠ **出るのが正解**） |
| `parts` / `drops` | 17本 | ⚠ **2本**（⚠ **どちらも意図的に壊しているぶん**） |
| `unlock` の赤 | — | ⚠ **1本**（⚠ **`E125` を意図的に出しているぶん**） |

⚠ **段階8で足した `ui_status_ch_st_rune_*` の15本が消えた。⚠ 人間が `ja.csv` を再インポートしたことがこれで確認できた。**


---

## 14. ⚠ 取り直した（**2026-08-24・人間が `.tres` を直したあと**）

⚠ **`initially_unlocked_screens` が `["adventure_select", "settings", "scenario"]` になった。** ⚠ **§12-6 の1（通っていない項目）が解消。**

```
--- 段階的に開くか ---
  最初            ["adventure_select", "settings", "scenario"]
  ⚠ initial_state_config.tres は直っている（最初から開くのは3つ）
  stage_1 クリア -> +["guild", "equipment", "training", "warehouse"]
  stage_2 クリア -> +["pomodoro", "decoration"]
  stage_3 クリア -> +["research", "shop", "rune"]
  workshop は開いたか -> false

--- 最初から（reset_to_new_game）---
  遊んだ状態  gold=100099 開いている画面=12件
  最初から後  gold=100     開いている画面=3件
```

| | 結果 |
|---|---|
| 全シナリオ（26本・`training` を除く） | ✅ **`red=0`（25本）／ `unlock` だけ `red=1`（意図的）** |
| 黄 | ✅ **平常 1本**（`skill_dbg_dot_odd`）／ ⚠ **`parts` と `drops` だけ 2本**（意図的） |
| `items` / `runes` | ✅ **`89 entries, 0 errors` ／ `25 entries, 0 errors`** |

⚠ **§6-A / §6-B / §6-C / §6-D は全項目通った。** ⚠ **残っているのは §7-B（画面・人間だけ）の12項目。**


---

## 15. ⚠ 画面の確認が通った（**2026-08-24・人間が実機で操作**）

⚠ **§7-B の12項目が全部 OK。** ⚠ **§7-A の3つ（`.tres` の編集・`ja.csv` の再インポート・新規開始）も済んでいる。**

⚠ **特に確認できた重いもの**：

| | 項目 | 結果 |
|---|---|---|
| ⚠ **詰みの回避** | ⚠ **装備も育成も閉じた状態で `stage_1` に勝てるか**（§7-B の最後・`GAME_DESIGN` 9-6） | ✅ **勝てた。⚠ §2 の解放の表を組み替える必要は無かった** |
| ⚠ **開いた瞬間** | ⚠ **`stage_1` を勝って拠点に戻った時点でギルドが出る**（⚠ **画面を出入りしなくても**） | ✅ |
| ⚠ **行の出し分け** | ⚠ **`stage_2` まで装飾の枠が出ない ／ `stage_3` までルーン枠だけ出ない** | ✅ |
| ⚠ **作業場** | ⚠ **最後まで出ない** | ✅ |
| ⚠ **レイアウト** | ⚠ **ボタンが減っても拠点の下段がはみ出さない** | ✅ |
| ⚠ **`F4`** | ⚠ **「画面を全部解放」で全部出る** | ✅ |

⚠ **段階9は完了。** ⚠ **§12-6 に「通っていない」と書いた5件のうち、⚠ 1・3・4・5 はこれで解消。⚠ 残る2（`unlock` の赤が1本出るのが正解）は仕様。**

⚠ **§9 の宿題1（どのステージで何が開くかが「勘」）は残る。** ⚠ **ただし実機で詰まらなかったので、⚠ ステージが増えるまで触らなくてよい。**
