...
### 1-1.【体制】§5〜§7 を実装役（MiniMax / Ziva）に渡す

**§4（分割の見本1本）・§8（`MasterDataLoader`）・§9（`training_screen.gd`）は設計役が書く。**

理由：`master_data_loader.gd` は424行、`training_screen.gd` も200行超で、**どちらも既存ファイルの途中の書き換え**。`AGENTS.md` の担当表で ❌ の枠。

⚠ **§5〜§7 は JSON と `ja.csv` だけで `.gd` はゼロ行。** `EXEC_SKILL_CONTENT.md` の回（`.gd` を1行も触らず18スキル）と同じ形。
