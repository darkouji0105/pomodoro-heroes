...
## 11-2. 検証用に追加したもの（**リリース前に消す**）

§7-7（装備を着けると軸が`(+N)`になる）を確認する手段が無かったため、デバッグオーバーレイを足した。

| ファイル | 内容 |
|---|---|
| `res://tests/debug_overlay.gd`（新規） | 全画面共通のデバッグパネル。`[F4]`で表示 |
| `res://autoload/scene_manager.gd` | `_ready()`と`_spawn_debug_overlay()`を追記（`OS.is_debug_build()`のときだけ生成） |

**キーは表示・非表示の`0`だけ。操作はすべてボタン。**

| 操作 | 効果 |
|---|---|
| **`0`キー** | **パネルごと表示・非表示**（見出しごと消える） |
| `▲ 畳む` / `▼ 開く` | 本体だけ開閉。畳むと見出し1行が残る |
| ゴールド・ジェム・スタミナ | 10万 / 1万 / 上限まで |
| 素材を全種類 | +999 |
| 消費アイテムを全種類 | +99 |
| **装備を全種類 1個ずつ** | **§7-7 はこれで確認する** |
| 研究を全部解放 | **先に「素材を全種類」を押すこと**（前提と素材の判定をそのまま通すため） |
| セーブする | `SaveManager.save_game()` |

**設計上の約束：**

- **資源もアイテムも`GameManager`の公開関数からしか配らない。** `_state`を直接書かない。特に装備は`add_to_inventory()`だけが個体（`eq_N`）を作る（`CLAUDE.md` 8番）。研究も`unlock_research_node()`を通すので、前提と素材の判定はそのまま働く
- **`.tscn`を作らない。** 消すときにシーン側の参照が残らない
- **新しいAutoloadを足さない。** `project.godot`と登録順のルール（`AGENTS.md`）を触ると消す手数が増えるため、既存の`SceneManager`にぶら下げた
- **キーは`0`の表示切替1つだけ。** `Input Map`（`project.godot`）は人間の担当で、検証のたびにキーを登録・確認するのは本題ではない。**操作系のキーをここに足さないこと。** `0`を選んだのは`BattleDebugPanel`が`1`〜`4`を使っていて`0`だけ空いているため
- **`0`が届かなくてもパネルを出せなくなることはない。** 畳むボタンが残る（`0`は`visible`、ボタンは本体だけ、と対象が違う）
- **`tr()`を通さない。** 開発者向けでリリースビルドには出ないため`ja.csv`に登録しない（`AGENTS.md`「`tr()`を使わないもの」。`BattleDebugPanel`と同じ扱い）。**素の`Button`を使い、`PrimaryButton` + `label_key`は使わない**
- **ボタンの`focus_mode`を`FOCUS_NONE`にしている。** 押したあとキーボードフォーカスがボタンに残ると、ゲーム側のキー操作（ポモドーロの`P`など）が効かなくなる

> **⚠ 経緯（同じ実装をやり直さないため）：** 最初は`F4`で開閉＋英字キーで操作する形にしたが、**`F4`だけ`_unhandled_input`に届かなかった**（観測した事実）。`project.godot`の Input Map に`F4`は無く、プロジェクト側のコードも握っていない。**外側の何が握っているかは特定できていない。** `F2`は届いた。**キーの当たり外れを調べるのが本題ではないので、全部ボタンに置き換えた。**

**消すときに触るのは上記2ファイルだけ。** `scene_manager.gd`は`_ready()`と`_spawn_debug_overlay()`と`DEBUG_OVERLAY_SCRIPT`の3ブロックを消せば元に戻る。
...
- **`res://tests/debug_overlay.gd`と`scene_manager.gd`のフックはリリース前に消す**（§11-2）
- `ja.csv`に`ui_common_yes`と`ui_common_no`が**重複して2行ずつある**。**今回の作業より前から存在する**（`git show HEAD`で確認済み）。今回は触っていない
...
| ファイル | 結果 |
|---|---|
| `scripts/utils/state_keys.gd` | 変更（`STAT_*`を4本→10本） |
| `autoload/game_manager.gd` | 変更（3箇所。`_stat_keys()`10本＋公開getter＋％軸の分類／`boost_all`を実数軸に限定／`SAVE_VERSION: 2`） |
| `autoload/save_manager.gd` | 変更（2箇所。`CURRENT_SAVE_VERSION: 2`／不一致で`return false`） |
| `scenes/guild/training_screen.gd` | 変更（ループ化＋`_stat_value_text()`を追加） |
| `scenes/guild/equipment_screen.gd` | 変更（3箇所。`_stat_labels()`を削除） |
| `resources/balance/master/characters.json` | 変更（3キャラに6軸） |
| `resources/balance/master/enemies.json` | 変更（3体に`mag`/`mdef`） |
| `localization/ja.csv` | 変更（6行追加・2行修正） |
| `resources/balance/initial_state_config.tres` | **人間が変更**（`save_version` = 2） |
| `tests/debug_overlay.gd` | **新規**（§11-2。指示書に無い。検証中に必要になった） |
| `autoload/scene_manager.gd` | 変更（§11-2のフック。指示書に無い） |

**§7（画面10項目）・§8（ログ5項目）・§9（セーブ4項目）を人間が確認した。**

### 13-1. 詰まった箇所（1件）

**§7-7（装備を着けると軸が`(+N)`になる）を確認する手段が無かった。** 装備を手に入れる経路が戦闘のドロップ・ショップ・作業場しかなく、検証のために毎回そこを通る必要があった。

→ **デバッグオーバーレイを追加した**（§11-2）。指示書の想定外だが、これが無いと§7-7 を確認できないため入れた。**リリース前に消す宿題として登録済み。**
...
**`F4`が`_unhandled_input`に届かない。** `project.godot`の Input Map に`F4`は無く（登録は`pomodoro_pause_toggle` = `P`のみ）、プロジェクト側のコードも握っていないが、実機で届かなかった。**`F2`と`0`は届いた。** 外側の何が握っているかは特定できていない。

**デバッグパネルにキーを割り当てるときは、届くかどうかを最初に確かめること。** 届かないキーを既定にすると「パネルが壊れている」ように見える。

### 13-4. 設計役が自分で見つけて直したミス（3件・実機に出る前）

デバッグオーバーレイを書いている途中で自分で見つけて直した。**記録に残すのは同じ形を繰り返さないため。**

1. **可視判定が実行より後にあった。** パネルを閉じていても資源が配られる状態だった。**「弾く判定は状態を変える前に置く」**（`CLAUDE.md` 6番）と同じ形
2. **`get_all_items()`をループの中で呼んでいた。** 1回転ごとに全アイテムの`duplicate(true)`が走る。`get_state()`と`get_research_tree()`にも同じ形があった
3. **`PRESET_TOP_RIGHT`を使っていた。** `CanvasLayer`は`Control`ではないので、子のアンカーが効かない

### 13-5. このタスクで確定した、次回に効く事実

- **`_stat_keys()`に足せば追従するのは合成側だけ。** 画面と戦闘は追従しない（§2-1）
- **`attack_interval_sec`と`cooldown_sec`は直読みだった**（§2-3）。予想は当たっていた
- **ダメージ経路は2箇所**（§2-4）
- **チャージスキルと`haste`の衝突は起きない**（§2-7）。最短CDは6.0秒で、`haste`100%でも3.0秒
