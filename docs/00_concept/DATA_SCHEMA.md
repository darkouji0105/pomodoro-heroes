...
- 初期値は `current: 20` / `max: 100`
- **`max`を大きく取る方針**：体験版のステージ消費は1回1のため、上限に当たることが実質起きない。ポモドーロの報酬が上限で切り捨てられて「やって損した」と感じる事態を避けるため（`CONCEPT.md`の設計原則）
- `add_stamina()`は`max`で**切り捨てる**。ただし上記のとおり実質発動しない
- 開始時を満タンにしないのは、最初のポモドーロ報酬が無意味に見えないようにするため
- ※ 将来「スタミナ回復用の消費アイテム（ポーション等）」に置き換える案もある。`PROJECT_STATUS.md`「横断的な未確定事項一覧」参照
...
- 具体的な中身・抽選テーブルは未確定。当面はどちらも汎用素材で埋め、`Balance`の`.tres`から調整する
  ...
  "session_history": [
	{
	  ...
	  "reward_given": { "stamina": 0, "chests": ["chest_type"] }
	}
  ]
}
```
...
- ポモドーロのスタミナ換算レート（作業1分あたり何スタミナか）の具体値
