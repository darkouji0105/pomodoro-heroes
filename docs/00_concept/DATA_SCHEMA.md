...
> **⚠ 作業場は作り直し対象**（`GAME_DESIGN.md` 9-3）。装備製作と素材変換が廃止され、中間素材と装飾のランダム製作に変わる。**`recipe_type`の`equipment`をどうするか、装飾のランダム製作をレシピの器に乗せるかは、作業場の作り直し時に決める**（5章）。

---

### 4-6. 装備の個体（`equipment_instances`）

```json
{
  "equipment_instances": {
	"eq_1": { "item_id": "armor_iron_helm", "grade": 1, "parts": [null, null] }
  },
  "next_equipment_instance_id": 2
}
```

- **IDは`eq_`＋連番。** `next_equipment_instance_id`は1から始まり、個体を作るたびに1増える
- **`parts`は`null`込みの長さ固定配列。位置が枠を表す。** 「刺さっているものだけ入れる」形にすると、どちらの枠か分からなくなる
- **装備中の個体も`equipment_instances`に残す。** 在庫から出し入れしない。`character_growth.<id>.equipment.<slot>`が`instance_id`を指すだけ
- **個体を作るのは`add_to_inventory()`を通る`_create_equipment_instance()`だけ。** 新しい入手経路を足すときは必ずここを通す。**直接`inventory`を書くと装備が個体にならず静かに消える**
- 性能値は状態に持たない。`item_id`の`equip_stats`×等級係数で毎回計算する
...
|---|---|
| `PART_SLOT_COUNT` | 2 |
| `MAX_EQUIPMENT_GRADE` | 3 |
| `PART_SLOT_GRADES` | `[5, 10]` |

> **⚠ 等級上限3 < 枠が開く等級5。いまのデータでは枠は永久に0個。** `get_open_part_slot_count()`は常に0を返す。**新設計では等級10・枠に種類つき**（5章）。

---

## 5. 新設計で増えるもの（**未実装**）

**ここに書かれているものは、まだ存在しない。** 1〜4章は実装済みの記録、この章はこれから増えるものの一覧。

**実装が済んだ項目は、5章から1〜4章へ移す。** 移すまでは「無い」。**この章を消さずに残すのは、どこまで実装されたかを1箇所で見えるようにするため。**

**構造の詳細はここに書かない。** 決まっているのは「何が要るか」までで、形は実装時に決める。中身は`GAME_DESIGN.md`の該当章が正。
...
| `parts`の要素を`{装飾ID, 等級, 出目}`のオブジェクトに | 7-6 |
| `MAX_EQUIPMENT_GRADE`を3 → **10**、`PART_SLOT_GRADES`を**種類つき**に | 6-2, 6-4 |
| 装飾のインベントリ（**種類＋等級のスタック品**。刺すまで個体を持たない） | 7-6 |
| ノルマトークン・鍛冶屋トークン・レア素材・ダンジョン用素材 | 2章 |
| ルーンの重ね段階（1〜5）とルーンのかけら | 7-7 |
| 周回ステージの解放状態、スキップ周回の記録 | 4-1 |
| 移動系ルーンの移動量設定（キャラプリセットに含む） | 7-7 |

**既存セーブは捨てる。移行処理は書かない**（`GAME_DESIGN.md` 14章）。ただし**キーが増えるだけのものは0埋め・空配列で落ちないこと**を確認する（`PLAN_IMPLEMENTATION.md` 1章）。

### この章に紐づく未決

- **装飾のランダム製作をレシピ（`recipes.json`）の器に乗せるか。** 乗せるなら`recipe_type`に`decoration`が要る。乗せないなら作業場専用の別構造になる
- **宝箱の中身の配分**（既存の未確定リストにもあるが、入手元ごとにテーブルを分ける方針は決まった。`GAME_DESIGN.md` 4-4・11-3）
