...
const EFFECT_TYPES_KNOWN: Array = [
	EFFECT_DAMAGE, EFFECT_HEAL,
	"buff", "dot", "dispel", "cancel", "transform", "move", "summon"
]
# 段階1で実際に当たるのはこの2つだけ。他は「書けるが飛ばす」（黄）。
const EFFECT_TYPES_IMPLEMENTED: Array = [EFFECT_DAMAGE, EFFECT_HEAL]
	...
	if not (raw_effects is Array) or (raw_effects as Array).is_empty():
		...
		for raw_effect: Variant in (raw_effects as Array):
			if not (raw_effect is Dictionary):
				...
				_validate_effect(issues, skill_id, raw_effect as Dictionary, index)
			index += 1
...
# effects[] の1要素ぶんの検証。
static func _validate_effect(issues: Array, skill_id: String, effect: Dictionary, index: int) -> void:
	...
	if not (effect_type in EFFECT_TYPES_IMPLEMENTED):
		_warn(issues, skill_id, "%s.type: '%s' は段階3以降。段階1では飛ばされる" % [where, effect_type])
	...
	if not (host in HOSTS_KNOWN):
		...
		_warn(issues, skill_id, "%s.host: '%s' は段階3。段階1では飛ばされる" % [where, host])
