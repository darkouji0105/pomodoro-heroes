...
# --- 鞄（2026-09-18・人間の決定「難ダンジョンのインベントリをシナリオでも適用」「とりあえず８枠」） ---
#
# ⚠ ルートの中で拾ったもの（宝箱・道中の戦利品）はまず拾い待ちに積まれ、⚠ 鞄に入れるかはプレイヤーが選ぶ。
# ⚠ ボスを倒すと鞄の中身を持ち帰る（⚠ 宝箱は拠点の宝箱、⚠ ほかは倉庫）。⚠ 負けて降りたら失う。
# ⚠ 形は難ダンジョンの鞄と同じ（⚠ 1個1枠・個数制限方式）。⚠ 値は仮置き。
