...
##   あるが、⚠ 1から3の中でランダム」。⚠ ＝**区画ごとに 1〜3 を振り、⚠ 出口側に寄せる**。
##   ⚠ 中の3層が確定で全部分かれていた形をやめた。
## ⚠⚠ 同じ日の決定38「⚠ 2〜4層つづく・1層だけの分かれ道にしない」を**一部覆している**。
##   ⚠ 戻すなら min を 2 に上げるだけでよい（⚠ コードは触らなくてよい）。
## ⚠ 振った値は区画の「中」の層数（＝`segment_layers` − 2）を超えない（⚠ 超えたら切り詰める）。
##   ⚠ 中の層数が min に届かないときだけ W35 が鳴く。
@export var segment_branch_min_layers: int = 1
@export var segment_branch_max_layers: int = 3
