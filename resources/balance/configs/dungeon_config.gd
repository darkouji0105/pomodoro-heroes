...
## 区画の「中」に置くノードの**合計**（⚠ 1層あたりではない）。
##
## ⚠⚠ 2026-09-20・人間の決定38：⚠ 「⚠ 5ノード直線の中に分岐で、3〜4ノードを入れる感じ」
##   ＋ ⚠ 「⚠ 分岐は区間の中の合計」。⚠ ＝中の3層に合計3〜4マスを置き、⚠ 分かれて合流する形にする。
##   ⚠⚠ これは段階20-j の「⚠ 分岐中は一本道のみでいい」（決定30）を**覆したもの**。
## ⚠ 配り方は真ん中の層から厚くする（⚠ 3層に4なら 1・2・1 ＝ 分かれて合流する）。
## ⚠ 中の層の数より小さくはできない（⚠ どの層にも最低1マス）。
## ⚠⚠ 増やすと区画帯の列が増え、⚠ 通常層（1マス）とのあいだで斜め線が長くなる
##   （⚠ 人間が実機で何度も指摘した症状。⚠ 台帳 §7 の「通常層を1マスのままにする」と同じ話）。
##   ⚠ 増やすときは `scenario=layout` の「一番斜めな線の横幅」を必ず見ること。
@export var segment_inner_total: int = 4
