...
## 区画の「中」を何本の道に分けるか（⚠ 分岐の本数）。
##
## ⚠⚠ 2026-09-20・人間の決定38：⚠ 「⚠ ぶんきは、1の長さしかないが2から4で」。
##   ⚠ ＝分かれたあと、⚠ **2〜4層ぶん並んで進んでから合流する**（⚠ 1層だけの分かれ道にしない）。
##   ⚠ 分岐の長さ＝区画の「中」の層数＝`segment_layers` − 2（⚠ 入口と出口のぶん）。
##   ⚠ だから `segment_layers` は 4〜6 にする（⚠ 5 なら分岐の長さ3）。⚠ 外れると W35 が鳴く。
## ⚠ 2 ＝ 二手に分かれる。⚠ 1 に戻すと一本道（⚠ 段階20-j の形）。
@export var segment_inner_ways: int = 2

## 分岐の長さ（層）の下限と上限。⚠ 外れたら W35 で知らせる（⚠ 生成は止めない）。
## ⚠ 人間の言葉「⚠ 2から4で」をそのまま持つ。
@export var segment_branch_min_layers: int = 2
@export var segment_branch_max_layers: int = 4
...
## ⚠⚠ 素材が1枠に何個まで重なるか（2026-09-20・人間の決定「⚠ 素材関連を１０まで」。⚠ 決定42）。
##   ⚠ 重なるのは**素材だけ**（⚠ ポーション・宝箱は1個1枠のまま＝人間の決定「ポーション以外をスタック」）。
##   ⚠ 1 にすると今までどおり「1個＝1枠」に戻る。
@export var bag_material_stack: int = 10
