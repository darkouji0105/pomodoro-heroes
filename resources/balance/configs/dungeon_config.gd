...
## 区画の「中」の層に何ノード置くか。
##
## ⚠⚠ 1 ＝ 区画の中は一本道（⚠ 人間の指示「⚠ 分岐中は一本道のみでいい」・段階20-j）。
##   ⚠ 決定28 の「中で分岐してもいい」を人間が覆した。
## ⚠⚠ 2 以上にすると区画帯だけノード数が増え、⚠ 全層の列数がそれに合わせて広がる。
##   ⚠ すると通常層（1マス）と区画帯のあいだで列が飛び、⚠ 斜め線が長くなる
##   （⚠ 人間が実機で何度も指摘した症状。⚠ 台帳 §7 の「通常層を1マスのままにする」と同じ話）。
## ⚠ 増やすときは `scenario=layout` の「一番斜めな線の横幅」を必ず見ること。
@export var segment_inner_nodes: int = 1
