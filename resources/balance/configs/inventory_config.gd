...
## ⚠ 500 ＝ 100 マス（10列 × 10行）× 5 ページ。⚠ 未決7 の裁きは「案②＝数百マス」。
...
# ⚠ 画面の幅に効く：⚠ 列 × 48px（⚠ 基準は 1280）。
#   ⚠ 列を増やすときは scenario=layout で最小幅を測り直すこと。
#
# ⚠⚠ 2026-09-08（人間の決定）：⚠ 20列 × 5行 → **10列 × 10行**。
#   ⚠ 右に常設の詳細パネル（300px）を置いたら、⚠ 20列（1042px）では
#   ⚠ 最小幅が 1358 になり 1280 を 78px はみ出した（⚠ scenario=layout の実測）。
#   ⚠⚠ **1ページ 100 マスは据え置き**（⚠ 10 × 10 = 100）。⚠ ページ数も
#   ⚠ 「枠を買う」で増える数（＝1ページぶん）も1つも変わらない＝バランスに触っていない。

## 1行に並べるマスの数。
@export var columns: int = 10

## 1ページの行数。
@export var rows_per_page: int = 10
