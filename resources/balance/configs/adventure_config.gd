...
#
# ⚠ 宝箱・レアリティ・たいまつ・フロア内ショップ・層の出現比は FloorConfig へ移した
#   （Balance.floor.chest_chance_pct のように参照する）。
# ⚠ このファイルは戦闘の見た目と上限を持つ場所として残す。フロアの経済を
#   ここに戻さないこと。混ぜると「バランスを見たい人が色の設定を読み飛ばす」形に戻る。
# ⚠ stamina_cost_per_stage だけはここに残した。フロアだけの話ではなく
#   「ステージに挑むコスト」で、stage_dbg_* にも効くため。
